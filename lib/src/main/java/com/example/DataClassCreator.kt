...
fun main(args: Array<String>) {
    parseObject("Parent", json)
}

fun parseObject(name: String, json: String) {
    try {
        val jsonObject = Gson().fromJson(json, JsonObject::class.java)
        parseObject(name, jsonObject)
...
fun parseObject(name: String, jsonObject: JsonObject) {
    val builder = StringBuilder("data class $name(")
    jsonObject.keySet()
            .forEach {
                builder.append("val $it :")
                val element = jsonObject[it]
                if (element.isJsonObject) {
                    builder.append(it)
                    parseObject(it, element.asJsonObject)
                } else if (element.isJsonArray) {
                    builder.append("List<$it>")
                    parseArray(it, element.asJsonArray)
...
fun parseArray(name: String, jsonArray: JsonArray) {
    parseObject(name, jsonArray.get(0).asJsonObject)
}